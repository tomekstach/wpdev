...
if( is_multisite() && !defined('WPW3ALL_MAIN_DBPREFIX')){ // compatibility for previous (<1.9.0) activated MUMS: they added this on wp-config.php
define( 'WPW3ALL_MAIN_DBPREFIX', $table_prefix ); // FOR WordPress MSMU MULTISITE
}
...
//add_shortcode( 'w3allforumtopic', array( 'WP_w3all_phpbb', 'wp_w3all_get_phpbb_topic_short' ) );
...
function w3all_xprofile_avatar_uploaded( $item_id, $avatar_data_type,  $avatar_data ) { 
 ...
 if(!empty($avaUrl)){ 
  ...
  $wpu_eh = WP_w3all_phpbb::w3all_phpbb_email_hash($wpu->user_email);
  $w3db_conn = WP_w3all_phpbb::wp_w3all_phpbb_conn_init();
	//$res = $w3db_conn->query("UPDATE ".$w3all_config["table_prefix"]."users SET user_avatar = '".$avaUrl."', user_avatar_type = 'avatar.driver.remote' WHERE user_email_hash = '".$wpu_eh."' OR username = '".$wpu->user_login."'");
