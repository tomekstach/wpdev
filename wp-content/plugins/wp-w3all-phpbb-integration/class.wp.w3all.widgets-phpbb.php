 ...
 if( get_option('w3all_iframe_phpbb_link_yn') == 1 ){ // IFRAME MODE LINKS
	...
	if( $w3all_custom_output_files == 1 ) { // custom file include
	   $file = ABSPATH . 'wp-content/plugins/wp-w3all-config/login_form_include_iframe_mode_links.php';
		...
		$file = WPW3ALL_PLUGIN_DIR . 'views/login_form_include_iframe_mode_links.php';
	}
} else { // NO IFRAME MODE LINKS
		if( $w3all_custom_output_files == 1 ) { // custom file include
	   $file = ABSPATH . 'wp-content/plugins/wp-w3all-config/login_form_include_noiframe_mode_links.php';
	} else { // default plugin file include
	 $file = WPW3ALL_PLUGIN_DIR . 'views/login_form_include_noiframe_mode_links.php';
	}
}
...
public function wp_w3all_phpbb_last_topics($post_text, $topics_number, $text_words) {
  ...
  global $w3all_url_to_cms, $w3all_get_phpbb_avatar_yn, $w3all_last_t_avatar_yn, $w3all_last_t_avatar_dim, $w3all_phpbb_widget_mark_ru_yn, $w3all_custom_output_files, $w3all_phpbb_widget_FA_mark_yn;
